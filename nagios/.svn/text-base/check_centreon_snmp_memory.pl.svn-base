...
my $DEBUG = 0;  #1 to enable debug print, 0 to disable
...
my $OID_Cache_storage = ".1.3.6.1.4.1.2021.4";
my $OID_Cache_storage_size = ".1.3.6.1.4.1.2021.4.15.0";
my $total_cache = $session->get_table(Baseoid => $OID_Cache_storage);
if (!defined($total_cache)) {
    printf("ERROR: Description Table Cache : %s.\n", $session->error);
    $session->close;
    exit $ERRORS{'UNKNOWN'};
}
...
if ($DEBUG == 1)
{
   print "Debug: Cache size: " . $cache_used . "\n";
   print "Debug: RealM size: " . $realM_size . "\n";
   print "Debug: RealM used: " . $realM_used . "\n";
   print "Debug: Swap size: " . $swap_size . "\n";
   print "Debug: Swap used: " . $swap_used . "\n";
   print "Debug: Total memory used: " . $total_memory_used . "\n";
   print "Debug: Total memory size: " . $total_memory_size . "\n";
}
...
if ($percent_used >= $opt_c){
    print "threshold (".$opt_c."%) excedeed : total memory used : ".$percent_used."%, ram used : ".$percent_realM_used."%, swap used : ".$percent_swap_used."% | used=".$total_memory_used." size=".$total_memory_size."\n";
    exit $ERRORS{'CRITICAL'};
} elsif ($percent_used >= $opt_w){
    print "threshold (".$opt_w."%) excedeed : total memory used : ".$percent_used."%, ram used : ".$percent_realM_used."%, swap used ".$percent_swap_used."% | used=".$total_memory_used." size=".$total_memory_size."\n";
    exit $ERRORS{'WARNING'};
} else {
    print "total memory used : ".$percent_used."%  ram used : ".$percent_realM_used."%, swap used ".$percent_swap_used."% | used=".$total_memory_used." size=".$total_memory_size."\n";
    exit $ERRORS{'OK'};
}
